...
### 1. Добавить пользователя и сразу получить готовый ключ
...
Если пользователь уже есть в конфигурации, UUID не меняется: скрипт восстанавливает или обновляет файл с текущими параметрами.
...
`xhttp-main` может отсутствовать в каталоге `/etc/xray-vless/users/`, если этот профиль создавался вручную. Это нормально и не означает, что пользователь отсутствует в конфигурации Xray.

### 3. Получить ключ пользователя
...
Или через специальный helper:

```bash
sudo bash scripts/show-user.sh Ivan
```

`show-user.sh` выводит имя, UUID, готовую `vless://`-ссылку и, если установлен пакет `qrencode`, показывает QR-код прямо в терминале.

Установка QR-кодировщика один раз:
...
можно использовать для быстрой передачи ключа на телефон: открыть QR-код в терминале и отсканировать его клиентом.
...
**v2rayNG — основной рекомендуемый вариант.** Для подключения достаточно импортировать персональную `vless://` ссылку или QR-код.
...
Либо передайте QR-код из `show-user.sh`, если установлен `qrencode`.
